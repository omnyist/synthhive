...
| `lizardroulette` | `LizardRouletteHandler` | Roll odds — lose and get timed out via Twitch Helix `POST /moderation/bans`. Tracks per-user death count and survival streak via `SkillStat` model. **Dynamic messages**: Success messages composed from fragment pools (`STREAK_TIERS`) — openers, bodies, and victim clauses picked randomly and joined. Four tiers escalate in belligerence as survival streak grows (1-2 chill, 3-4 annoyed, 5-7 angry, 8+ furious). **Victim callouts**: `_last_victim` (in-memory) tracks last death per channel; survivors' messages reference the previous victim. **Streak tracking**: `stats["streak"]` persisted in SkillStat, incremented on survival, reset on death. `$(streak)` available in failure messages for broken streak count. **Bullet mechanic**: `LizardBullets` component silently rolls 1/651 every 30s per channel (live only) to load a 6-chamber gun — when loaded, next 6 uses are guaranteed losses. Config: `odds` (1-100), `failure`/`failure_first`/`timeout_failed` messages, `timeout_duration`, `timeout_delay` (default 5s), `cooldown`. Per-user cooldown scoped per-channel. |
