...
- **Skill** — A Python-coded command toggled per channel. Used for complex built-in behaviors that need real Python logic (future: quotes, followage, API integrations). Logic lives in `bot/skills/` as handler classes, the model controls enable/disable and stores JSON config. Configurable skills declare a pydantic `config_schema` on the handler (`extra="forbid"`); config is validated at every write path (API + admin via `core/config_validation.py`), persisted sparsely (`exclude_unset` — defaults live in code, not rows), and schemas are served at `GET /api/v1/skills/schema/` for form rendering. Command-type configs (text/lottery/random_list/counter) are validated the same way. Code defaults are tenant-neutral: channel flavor (emotes, currency, easter eggs) lives in Skill rows (existing tenants' flavor frozen by migration 0016).
...
- **SkillStat** — Per-user stats for a skill in a channel, keyed by `(channel, skill_name, twitch_id)`. Counter stats (`plays`, `deaths`, `survivals`, `streak`, `max_streak`, `bullet_deaths`, `streaks_broken`) are real integer columns updated atomically with `F()` and ordered in the DB for leaderboards; `last_mood` is a CharField. The `stats` JSONField remains only for future non-counter per-skill data.
...
- **DungeonWager** — Append-only journal of every dungeon wager, written at debit time. Statuses: `pending` → `won`/`lost` at resolution; rows still `pending` at bot startup are orphans from a killed game and are refunded by the `DungeonRecovery` component. The durable money record for a game whose live state is an in-memory asyncio task.
...
| `lizardroulette` | `LizardRouletteHandler` | Roll odds — lose and get timed out via Twitch Helix `POST /moderation/bans`. **Mood engine** (`bot/skills/lizardmood.py`): Each play rolls a weighted random mood (theatrical, bored, impressed, smug, clinical, gleeful, deadpan) that controls message tone, countdown style, timeout delay multiplier, and victim clause inclusion. Context (death count, streak, bullets loaded, first death) shifts mood weights but any mood can appear. `roll_mood()` returns `MoodRoll` (mood + weights + context) for ML training data. Accepts optional `weight_fn` callback for future ML override. **Message pools**: Per-mood, per-tier message fragments. Survival: opener + body + victim clause (compositional). Death: core + countdown (from mood behavior). **Stats tracked** in `SkillStat.stats` JSONField: `deaths`, `streak`, `max_streak`, `plays`, `survivals`, `bullet_deaths`, `streaks_broken`, `last_mood`. **Victim callouts**: `_last_victim` (in-memory) tracks last death per channel; moods with `include_victim_clause=True` reference the previous victim. **Bullet mechanic**: `LizardBullets` component silently rolls 1/651 every 30s per channel (live only) to load a 6-chamber gun — when loaded, next 6 uses are guaranteed losses. Config: `odds` (1-100), `timeout_failed` message, `timeout_duration`, `timeout_delay` (default 5s, multiplied by mood), `cooldown`, `bullets_enabled`. Per-user cooldown scoped per-channel. |
