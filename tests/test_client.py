...
async def test_before_invoke_releases_the_connection():
    ...
    in CommandRouter's release_connection() call runs on that path. before_invoke
    is the framework's one hook that runs for every registered command
    regardless of which Component defined it, so it's the only place this
    can be verified without exercising a live command end-to-end.
    """
    bot = BotClient.__new__(BotClient)

    with patch("bot.client.release_connection", new=AsyncMock()) as released:
